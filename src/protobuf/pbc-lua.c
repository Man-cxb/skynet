...
_env_new(lua_State *L) {
	...
	lua_getfield(L, LUA_REGISTRYINDEX, "PROTOBUF_ENV");
	if (lua_type(L, -1) == LUA_TLIGHTUSERDATA)
	{
		env = (struct pbc_env *)lua_touserdata(L, -1);
		lua_pop(L, 1);
	}
	else
	{
...
_gc(lua_State *L) {
	lua_settop(L,1);
	struct pbc_env * env = (struct pbc_env *)lua_touserdata(L,1);
	if (env->gcobj == NULL)
	{
		struct gcobj * obj;
	...
	{
		lua_pushlightuserdata(L, env->gcobj);
	}
